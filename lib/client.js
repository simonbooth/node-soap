...
Client.prototype._defineMethod = function(method, location) {
  var self = this;
  var temp;
  return function(args, callback, options, extraHTTPHeaders, extraSOAPHeaders) {
    if (typeof args === 'function') {
      ...
      callback = options;
      options = temp;
    } else if (typeof extraHTTPHeaders === 'function') {
      temp = callback;
      callback = extraHTTPHeaders;
      extraHTTPHeaders = options;
      options = temp;
    }
...
Client.prototype._invoke = function(method, args, location, callback, options, extraHTTPHeaders, extraSOAPHeaders) {
  var self = this,
    name = method.$name,
    input = method.input,
  ...
  if (typeof extraSOAPHeaders === 'object') {
    extraSOAPHeaders = this.wsdl.objectToXML(extraSOAPHeaders);
  }
  ...
  for (var attr in extraHTTPHeaders) { headers[attr] = extraHTTPHeaders[attr]; }
  ...
  xml = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
    ...
    ((self.soapHeaders || self.security || extraSOAPHeaders) ?
      (
        "<" + envelopeKey + ":Header>" +
        (self.soapHeaders ? self.soapHeaders.join("\n") : "") +
        (extraSOAPHeaders ? extraSOAPHeaders : "") +
